...
sub doFlightSelection {
	...
	printSelectionHeader();
  	...
  	print "\n</blockquote>\n";
  	print p,p,p,"The Variables are: ",br,dumpVars() if ($userID eq "vars");
  	print end_html;
...
sub doIntro {
  ...
  printSelectionForm();
  print "\n</blockquote>\n";
  print p,p,p,"\nThe Variables are: ",br,dumpVars() if ($userID eq "vars");
  print end_html(), "\n";
}
...
sub printSelectionForm {
	...
	print start_form(-method=>'POST', -action=>'reservations.pl',
					-name=>'DestForm',
					-onSubmit=>"$jsCode"),
...
sub printFlightChoiceForm {
	...
	print start_form('POST', 'reservations.pl');
	print "\n<center>";
...
EOF
		...
		print start_form(-name=>"JSFormSubmitForm2", 
...
sub showPurchaseSummary {
	...
	print OUTPUT start_form(-method=>'post', -action=>"$resScript"),
