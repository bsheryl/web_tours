...
sub doGetInfo {
	...
	print	start_form(-method=>"POST", -target=>"info",  -action=>"login.pl", -onSubmit=>$JScript);
	print
		table(
		...
		end_form(), "</table></blockquote>", end_html(), "\n";
...
sub doBadAccess {
	...
	print p,p,p,"The Variables are: ",br,dumpVars() if (param('username') eq "vars");
	print end_html();
